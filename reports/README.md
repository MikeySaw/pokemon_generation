...
- Student 12285004 was responsible for setting up the initial cookie cutter project structure. They ensured that the development environment was standardized and reproducible. Developed configuration files, contributed to (CI/CD) pipeline, setting up GitHub Actions for automated testing, linting, and quality checks quality checks using pytest. Contributed to this document as well. Contributed to deploy model and dockerfiles to the cloud. Contributed to several model deployment files. Contributed to Lightning Model Inference.
