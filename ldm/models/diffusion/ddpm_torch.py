...
import numpy as np
from torch.optim.lr_scheduler import LambdaLR # define a customized learning rate scheduler
...
from ldm.utils import exists, default, count_params
...
CONDITIONAL_KEYS = {'concat': 'c_concat',
...
class LatentDiffusion(DDPM):
    """
    Latent Diffusion Model for fine-tuning a pre-trained DDPM model. 
    """
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
        ...
        try:
            self.num_downs = len(first_stage_config.params.ddconfig.ch_mult) - 1 # number of downsampling blocks in latent diffusion model
        except:
            self.num_downs = 0
        if not scale_by_std:
    ...
    def batch_start_logic(self, batch, is_first_batch=True):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.use_positional_encodings:
                ...
                ckey = CONDITIONAL_KEYS[self.model.conditioning_key]
