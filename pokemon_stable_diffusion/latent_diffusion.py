...
import torch
import torch.nn as nn  # TODO: imported but unused
...
from ldm.utils import default, instantiate_from_config, isimage, ismap, log_txt_as_img
import wandb
...
def save_image(img, path):
    img = 255.0 * rearrange(img.cpu().numpy(), "c h w -> h w c")
...
class LatentDiffusion(DDPM):
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            assert c is not None, "Conditioning is required for this model."
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if (
                self.cond_stage_key in ["image", "file_name", "segmentation", "bbox_img"]
...
# test the model with a dummy input tensor
if __name__ == "__main__":
    ...
    # print(c.device, "this is the device the text embeddings are on")
    # print(dummy_images.device, "this is the device the dummy images are on")
