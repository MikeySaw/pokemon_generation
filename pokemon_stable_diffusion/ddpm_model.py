...
from ldm.modules.distributions.distributions import DiagonalGaussianDistribution, normal_kl  # TODO: imported but unused
from ldm.modules.ema import EMA
from ldm.utils import count_params, default, exists, instantiate_from_config, isimage, ismap, log_txt_as_img
import wandb

# Add the working directory to the python file search path
# Docker does not do this automatically
# This file has to be run from the parent directory of pokemon_stable_diffusion/ (the project root)
...
class DDPM(nn.Module):
    """
    Define the DDPM model backbone for model training and sampling.
    """
    ...
    def __init__(
        ...
        parameterization="eps",  # this parameter controls whether we will predict x0 or eps
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        except:  # TODO: change to except SomeError: ... to avoid linting error
            self.num_downs = 0
        if not scale_by_std:
    ...
    @torch.no_grad()
    def log_images(  # TODO: log_images is defined twice, one has to go
        self,
        batch,
        N=8,
        n_row=4,
    ...
    def configure_optimizers(self):
        lr = self.scheduler_config["optimizer"]["lr"]
        params = []
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, **kwargs):
        log = super().log_images(batch, N=N, n_row=n_row, sample=sample, **kwargs)
        images = {key: wandb.Image(image) for key, image in log.items() if isinstance(image, torch.Tensor)}
        wandb.log(images)
        return log


# use a dummy input to test if there are bugs inside the model
